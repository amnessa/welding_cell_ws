...
### 1.2.1 The rebuilt corpus (`out/bench_phase4_visfix`, 2026-09-11 19:36)

Chain: build 2 h 45 min (720 scenes, no stratum short), fixture twins (274, as before), rule
blocks, D34 PASS (worst 0,050 mm), `verify` green on all classes, meshes emitted. Compared
scene by scene with `out/bench_phase4`:

| stratum | ids | `visible_from_cam` changed (mean / worst scene) | `exterior` changed | seam labels changed |
|---|---|---|---|---|
| T/line, butt ×3, corner, edge, lap | identical | **0 / 0** | 0 | none |
| butt/arc | identical | 0,0 % / 0,5 % | 0,1 % | fractions only |
| T/swept_path | identical | 0,4 % / 3 % | 0,5 % | fractions only |
| T/circle | 4 replaced | 2,3 % / 12,5 % | 1,0 % | fractions only |
| T/ellipse | 8 replaced | 1,8 % / 11,6 % | 2,2 % | fractions **+ 12 scenes: an inner-ring seam weldable → `bisector_blocked`** (the D4 cone verdict, now exact) |
| T/saddle | 1 replaced | 3,7 % / 12,0 % | 2,1 % | fractions + `clear_fraction` in 49 |
| T/rounded_rect | 6 replaced | 4,1 % / 16,4 % | 2,2 % | fractions |

The 19 replaced ids are seeds that only passed `NoVisibleSeams` because of the old bug;
the next seeds in each residue took their place, so every stratum still has 60.

**Twin gate on the rebuilt corpus, one scene per stratum: 12/12 PASS.** Coverage recall is
≥ 0,994 on every stratum (it was 0,29 on rounded_rect and 0,84 on circle before the fix),
residual p99 ≤ 0,033 mm, object ids ≥ 0,999; `out/pilot_2026-09-11/gate_visfix_12strata.json`.

**A second finding, about the determinism gate.** Every plate stratum re-hashed too, with
*no* array or label change: the old corpus was generated under numpy 2.3.5 and this one under
2.5.3, and the last digits of a handful of floats differ (max relative 7e-6 on values near
zero, 1e-15 elsewhere). SCHEMA.md §6.2's "identical hash on any machine" therefore holds only
under a pinned numpy; the honest statement is "identical under the pinned environment,
semantically identical otherwise". Open decision added to `dataset_plan.md` §10: pin numpy
(and trimesh) for the release, or define the content hash with a float tolerance.

**Recommendation to the user:** swap — `bench_phase4` → `bench_phase4_pre_visfix` (kept as the
Phase 4 provenance), `bench_phase4_visfix` → `bench_phase4` (same for `_fx`); re-run the Phase 4
method batch for the six affected strata (circle, ellipse, saddle, rounded_rect, swept_path,
butt/arc ≈ 35 h); regenerate `facts.csv` and the gate report; keep the plate-strata batch
results (their arrays are unchanged, only hashes moved).

### 1.2.2 Swap done, Phase 4 re-run started (2026-09-11, user's go)

* `out/bench_phase4` → `bench_phase4_pre_visfix` (+ `_fx`), the rebuilt corpus is now
  `out/bench_phase4` (+ `_fx`); `facts.csv`, the gate report and the retired-ids file were
  carried over (they are regenerated by the batch).
* **Were the pre-fix Phase 4 numbers healthy?** Per stratum, from what the harness feeds the
  methods (`baselines.dataset.cloud_for`: "single" = the `visible_from_cam` points, "full_exterior"
  = the `exterior` points): the six plate strata never changed a single array or label → their
  numbers stand exactly. The six tube / swept strata (circle, ellipse, saddle, rounded_rect,
  swept_path, butt/arc) fed the methods points a camera cannot see (up to 16 % of a scene) and
  slightly wrong exterior sets → forfeit for those strata. Re-run in the batch's own self-healing
  mode: `--retire` (rows of the 360 pre-fix scene ids of those strata archived to
  `retired_rows_*.csv.gz`, concat rewritten) then `--append-missing` (every chunk re-runs only
  the scenes without rows: the 341 regenerated + 19 new ids; fixture chunks are paired runs and
  keep their files — their T rows are stale and are the one thing left to redo by hand:
  delete `fixture_*.csv.gz` and re-run `--only fixture`). Estimated ≈ 17 h here; resumable.
* The seven literature methods are the ones re-run; radius-PCA (`ours`) is excluded from the
  batch by design, as before.
* Regenerated for the new corpus: `out/bench_phase4/gate_report_2026-09-11.txt` (D34 PASS,
  D28 reported) and `facts.csv` from `baselines.dataset.scene_facts` (same 23 columns; on the
  701 shared scenes only `mean_occluded_fraction`, `best_visible_fraction` and `n_primary`
  moved, all in the tube strata). The paired fixture chunks are queued to re-run automatically
  after the self-healing run (`scratchpad/phase4_fixture.sh`; the pre-fix fixture files go to
  `phase4_batch/pre_visfix_fixture_chunks/`). Then: rebuild notebook 15 (`build_15_phase4_results.py`
  + `nbconvert --execute`), re-execute 13, revise the prose cells whose numbers moved, update
...
**M5 — materials + lighting substream** (2–3 days; the variance) — **landed 2026-09-11** on the
user's rule "published or structured, never tuned by eye":
* `scripts/fetch_render_assets.py` pulls **CC0** assets by name through the sites' public APIs
  into `out/render_assets/` with a manifest (URL, licence, md5/sha256, size per asset; per-HDRI
  luminance statistics; `set_hash` → part of `render_id`): 24 Poly Haven HDRIs (machine
  shops, workshops, garages, hangars, 2k) and 21 ambientCG surface sets (colour, roughness,
  normal maps) grouped by surface condition, after the user dropped the eight heavy-corrosion
...
* `render/materials.py` = `materials-1.0`: alloy F0 from Real-Time Rendering 4e Table 9.2
  ...
  per-part jitter, normal map, metallic per condition). `usd_stage.planar_st` gives every
  triangle texture coordinates by planar projection in the part frame at the set's physical
  tile size; `_textured_pbr` builds the UsdPreviewSurface network.
* `render/hdr.py`: a pure-numpy Radiance reader, luminance statistics, and the **dome
  intensity rule** — measured, because the first render under HDRIs came out black: this
  renderer's dome wants intensities in the hundreds (tonemapper response measured at
  300 → frame mean 115/255, 1000 → 190, 3000 → 234), so intensity = 300 ×
  (0,75 / the HDRI's mean luminance) × the exposure draw; LDR lab panoramas 900 × exposure;
  the key light is a fill at 100–600. Draws appended (order 6–7 in `draws.py`): per-part
  surface set, uv rotation, uv offset, roughness jitter; dome rotation.
* Checks: 8 scenes × 2 views under the full stack — every twin-gate residual identical to the
  material-free run (materials do not move the ray cast), frame means 69–140/255, masks
  unaffected. Tests: 5 new pure tests (render_id includes the asset hash, F0 table, draw
  domains and append-only prefix, planar st, HDR round trip + dome rule).
* **Not done, on purpose:** the "specular dropout / shadow holes" sensor rule from the
  original M5 list. Without measurements from the real camera it would be a tuned guess,
  which is exactly what the user ruled out; it stays a documented opt-in for Phase 9 once the
  `d435i` flat-target measurements exist (§10 of `dataset_plan.md`).
...
**M6 — batch render** (an evening) — **started 2026-09-11**: the 3600-scene `out/train_v1`
(12 strata × 300, base seed 3 000 000 so no seed overlaps the benchmark) is building as twelve
parallel per-stratum jobs (`make_bench6b.py --classes/--sources/--base-seed`, merged by
`--merge`), followed by rule blocks, the D28 gate, `verify` and meshes; then
`scripts/render_tier2.py` over it with `views: 10` (36 000 frames, `--resume`, one Isaac app).
Corpus done 2026-09-12 03:14 (build 7 h 03 min as twelve parallel jobs; 20 788 tacks; D34 PASS;
`verify` green on all 3600; 7200 meshes; 16 GB). Two fixes on the first render hour: an
*undrawable* view (no camera found in 40 attempts) has no files and crashed `render_hash` —
now hashed by name only, as `verify_render` already did; and the per-source manifest parts of
the three plate classes clashed on the name `line` — parts are now named by class and source,
and a lost part is reconstructed from the class index. Render relaunched with `--resume`.

**Gate refinements forced by the training corpus (2026-09-12, first 170 scenes):** 12 of the
first 57 *edge* scenes failed the twin gate with recall 1,0 and residual 0,0006 mm — i.e. on
definitions, not data. Two plates 2 mm thick face to face, seen nearly edge-on:
(1) the coverage range test let hidden faying-surface points count as rendered → "rendered
visible" now also requires the point's normal to face the camera and the id buffer at its
pixel to show its own object (coverage 0,67–0,92 → 0,995–0,999, recall unchanged);
(2) the object statement compared the id buffer with the nearest tier-1 *sample*, and at
0,25 pts/mm² the nearest sample to a pixel on a 2 mm plate can lie on the plate behind it →
it now uses the nearest exact *mesh*, excluding a 1 mm band around the seams where two
surfaces meet (0,96 → 1,000). A third render-hour bug: the id-buffer lookup table was sized by the ids *present in the
frame*, and a prim fully hidden in a view still has a declared id → `IndexError` on an edge
scene (`replicator.labels_from_ids`, now sized by the declared ids, with a test). The
