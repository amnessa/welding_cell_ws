...
Two things differ from the sim launch:

  * The real ``realsense_camera_node.py`` already publishes CameraInfo from the
    device intrinsics and stamps the images with ``camera_color_optical_frame``,
    so there is no relay node to rewrite frame_id / synthesize CameraInfo.
  * TF comes from the real driver's ``/joint_states`` on the wall clock, so
    every node runs with ``use_sim_time:=false``.
    ...
    realsense_camera_node  (librealsense)
        /camera/color/image_raw       (rgb8)
        /camera/depth/image_rect_raw  (16UC1, mm, aligned to color)
        /camera/color/camera_info

    depth_image_proc::PointCloudXyzrgbNode
        /camera/depth/color/points  (PointCloud2, XYZRGB)  in camera_color_optical_frame

Set the RViz Fixed Frame to ``base_link`` and add a PointCloud2 display on
...
-----
    ...
    # camera already running elsewhere (e.g. the realsense2_camera driver)
    ros2 launch admittance_control pointcloud.launch.py launch_camera:=false

    # the UR driver's own robot_state_publisher is already up
    ros2 launch admittance_control pointcloud.launch.py launch_robot_state:=false
...
# the arm, so it must match the camera node's frame_id AND the extrinsic
...
def _default_rviz_config() -> str:
    ...
    launch ...)`` copies the file at build time. Re-saving the layout from RViz
    writes wherever you point the save dialog, so a config edited in ``src/``
...
def _rviz_arguments(context) -> list:
    ...
    if not os.path.exists(config):
        print(f"[pointcloud.launch.py] rviz_config {config} does not exist; "
              f"starting RViz with its default layout. If you just edited the "
              f"config under src/, rebuild the package.")
        return []
    return ["-d", config]
...
def launch_setup(context, *args, **kwargs):
    ...
    # ---- Robot TF: base_link -> ... -> tool0 from the real driver's joint states ----
    if launch_robot:
    ...
    # ---- RealSense: RGB + depth (aligned to color, 16UC1 mm) + CameraInfo ----
    nodes.append(Node(
        package="admittance_control",
        ...
        parameters=[{
            ...
            "color_width": int(LaunchConfiguration("camera_width").perform(context)),
            "color_height": int(LaunchConfiguration("camera_height").perform(context)),
            "depth_width": int(LaunchConfiguration("camera_width").perform(context)),
            "depth_height": int(LaunchConfiguration("camera_height").perform(context)),
    ...
    nodes.append(ComposableNodeContainer(
        ...
        composable_node_descriptions=[
            ComposableNode(
                ...
                parameters=[{"queue_size": 10, "use_sim_time": False}],
            ),
        ],
    ))
...
def generate_launch_description():
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description="Start robot_state_publisher on /joint_states. Set false "
                        "if the UR driver or the main admittance_control launch "
                        "already runs it."),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            description="RealSense serial number. Empty = first device found."),
        DeclareLaunchArgument(
            "camera_width", default_value="1280",
            description="Color and depth stream width (px)."),
        DeclareLaunchArgument(
            "camera_height", default_value="720",
            description="Color and depth stream height (px)."),
        DeclareLaunchArgument(
            "camera_fps", default_value="30",
            description="Color and depth stream frame rate (Hz)."),
        DeclareLaunchArgument(
        ...
        DeclareLaunchArgument(
            ...
            description="Ground plane height in base_link: points with z <= this "
                        "are dropped. The real bench and the sim floor differ, so "
