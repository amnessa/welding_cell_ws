...
| `~/save_object` (icp_pose_refiner) | `std_srvs/Trigger` | Freeze the tracked CAD into the SEPC at the robust mean of the last `save_pose_window` tracked poses (the tracker wanders 5 mm / 6° on a stationary part; the spread goes into `assembly.json` as `pose_stats`); persist `static_env.*` + `assembly.json`; clear tracking for the next part. Keep the arm still a few seconds first. |
